		...
		while (i < length) {
			...
			switch (opcode) {
				...
				case Commands.DrawRect: {
					if (!driver.drawRect) {
						throw new Error("WebGPU backend does not implement 'drawRect()'.");
					}
				...
				case Commands.UpdateView: {
					if (!driver.updateView) {
						throw new Error(
							"WebGPU backend does not implement 'updateView()'.",
						);
					}
					...
					this.cameraPos3 = [data[i++]!, data[i++]!, data[i++]!];

					driver.updateView();
					break;
				}

				case Commands.DrawMesh: {
					const meshId = data[i++]!;
				...
				case Commands.SetDepth: {
					if (!driver.setDepth) {
						throw new Error("WebGPU backend does not implement 'setDepth()'.");
					}
					driver.setDepth(data[i++]!);
					break;
				}
			}
		}
